...
// Progress-Based ControlsDlg.cpp : implementation file
//

#include "pch.h"
#include "framework.h"
#include "Progress-Based Controls.h"
#include "Progress-Based ControlsDlg.h"
...
CProgressBasedControlsDlg::CProgressBasedControlsDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_PROGRESSBASEDCONTROLS_DIALOG, pParent)
{
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_PROGRESS2, m_progBarCtrl);
	DDX_Control(pDX, IDC_PROGRESS1, m_progBarCtrl1);
}
...
BOOL CProgressBasedControlsDlg::OnInitDialog()
{
	...
	for (int i = 0; i <= 100; i++) {
		m_progBarCtrl.SetPos(i);
	}

	m_progBarCtrl1.SetRange(0, 100);

	for (int i = 0; i <= 80; i++) {
		m_progBarCtrl1.SetPos(i);
	}
